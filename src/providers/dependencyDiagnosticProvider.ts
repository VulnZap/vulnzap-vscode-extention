...
export class DependencyDiagnosticProvider implements vscode.Disposable {
  ...
  private securityWebviewProvider?: any; // Will be set by extension.ts
  ...
  updateDependencyDiagnostics(
    ...
    if (this.securityWebviewProvider) {
      this.securityWebviewProvider.updateDependencyVulnerabilities(scanResult);
    }
  }
   ...
   */
  private async updatePackageJsonDiagnostics(
    ...
    for (const folder of workspaceFolders) {
      const packageJsonUri = vscode.Uri.file(
        `${folder.uri.fsPath}/package.json`
      );
      ...
      try {
        ...
        const diagnostics = this.createDiagnosticsForDocument(
          document,
          vulnerabilities
        );
   ...
   */
  private createDiagnosticsForDocument(
    ...
    for (const vuln of vulnerabilities) {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (match) {
          ...
          const message = `${vuln.severity.toUpperCase()} vulnerability in ${
            vuln.packageName
          }@${vuln.packageVersion}: ${vuln.description}`;
  ...
  /**
   * Links this provider with the security webview for synchronized updates
   */
  setSecurityWebviewProvider(provider: any) {
...
class DependencyCodeActionProvider implements vscode.CodeActionProvider {
  constructor(
    private dependencyDiagnosticProvider: DependencyDiagnosticProvider
  ) {}
  ...
  provideCodeActions(
    ...
    for (const diagnostic of dependencyDiagnostics) {
      ...
      const packageMatch = diagnostic.message.match(
        /vulnerability in ([^@]+)@/
      );
      if (packageMatch) {
        ...
        if (workspaceFolders) {
          ...
          const vulnerabilities =
            this.dependencyDiagnosticProvider.getVulnerabilitiesForProject(
              projectPath
            );
          ...
          if (vuln) {
            const quickFixes =
              this.dependencyDiagnosticProvider.getQuickFixesForVulnerability(
                vuln,
                document
              );
