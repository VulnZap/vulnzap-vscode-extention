...
class SharedPromptBuilder {
  ...
  static buildSecurityAnalysisPrompt(
    ...
    const scanType = fastScan
      ? "Fast security scan"
      : "Comprehensive security analysis";
    const focusLevel = fastScan
...
- Ignore console.log statements UNLESS they contain sensitive data (API keys, passwords, tokens, secrets)
...
export class VulnZapProvider implements APIProvider {
  ...
  private async performTextBasedAnalysis(
    ...
    const prompt = SharedPromptBuilder.buildSecurityAnalysisPrompt(
      code,
      language,
      {
        maxLines: 200,
  ...
  private validateAndNormalizeResponse(data: any): AISecurityResponse {
    try {
      ...
      const normalizedResponse: AISecurityResponse = {
        issues: Array.isArray(parsedData.issues)
          ? parsedData.issues.map((issue: any) => ({
              line: Math.max(0, Number(issue.line) || 0),
              column: Math.max(0, Number(issue.column) || 0),
              endLine: Math.max(
                0,
                Number(issue.endLine) || Number(issue.line) || 0
              ),
              endColumn: Math.max(
                0,
                Number(issue.endColumn) || Number(issue.column) || 0
              ),
      ...
      return SharedPromptBuilder.filterLowConfidenceIssues(
        normalizedResponse,
        70
      );
    } catch (error) {
