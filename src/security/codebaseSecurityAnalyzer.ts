...
export class CodebaseSecurityAnalyzer {
    ...
    private async performAPIAnalysis(
        ...
        try {
            ...
            const parsedIssues = this.convertAIResponseToSecurityIssues(aiResponse, filePath);
            return parsedIssues;
        } catch (error) {
    ...
    private convertAIResponseToSecurityIssues(
        ...
        if (aiResponse && aiResponse.issues && Array.isArray(aiResponse.issues)) {
            for (const issue of aiResponse.issues) {
                ...
                Logger.debug(`SecurityIssue: ${securityIssue.type} at ${securityIssue.startLine}:${securityIssue.startColumn}-${securityIssue.endLine}:${securityIssue.endColumn}`);
                issues.push(securityIssue);
            }
        }

        Logger.debug(`Converted ${issues.length} API issues to SecurityIssue format`);
        return issues;
    }
