...
export class DependencyParser {
    private ecosystems: EcosystemInfo[] = [
        ...
        {
            name: 'pip',
            files: ['requirements.txt', 'requirements-dev.txt', 'requirements-test.txt', 'Pipfile', 'pyproject.toml'],
     ...
     */
    async scanWorkspaceForDependencies(workspaceFolders: readonly vscode.WorkspaceFolder[]): Promise<Dependency[]> {
        const allDependencies: Dependency[] = [];

        for (const folder of workspaceFolders) {
            const dependencies = await this.scanFolderForDependencies(folder.uri.fsPath);
    ...
    private async getAllFiles(dirPath: string): Promise<string[]> {
        const files: string[] = [];
        const ignoreDirs = new Set(['node_modules', '.git', 'dist', 'build', 'target', 'vendor', '__pycache__', '.venv']);
     ...
     */
    private parsePythonDependencies(content: string, filePath: string): Dependency[] {
                    ...
                    if (trimmed && !trimmed.startsWith('#') && !trimmed.startsWith('-')) {
                        ...
                        if (match) {
                            dependencies.push({
                                ...
                                isDevelopment: fileName.includes('dev') || fileName.includes('test'),
                                filePath
                            });
                        }
                    }
    ...
    private parsePipfile(content: string, filePath: string): Dependency[] {
        ...
        for (const line of lines) {
            ...
            } else if ((currentSection === 'packages' || currentSection === 'dev-packages') && trimmed.includes('=')) {
    ...
    private parseGoMod(content: string, filePath: string): Dependency[] {
        ...
        for (const line of lines) {
            ...
            if (trimmed.startsWith('require ') || inRequireBlock) {
                const requireLine = trimmed.startsWith('require ') ? trimmed.substring(8) : trimmed;
                    ...
                    if (trimmed && trimmed.includes('=') && !trimmed.startsWith('#')) {
                        ...
                        if (name && version) {
                            dependencies.push({
                                ...
                                packageName: name.trim(),
                                packageVersion: this.cleanVersion(version.trim().replace(/"/g, '')),
                                isDevelopment: section.includes('dev') || section.includes('build'),
                                filePath
                            });
                        }
                    }
            ...
            if (trimmed.startsWith('gem ')) {
                ...
                if (gemMatch) {
                    dependencies.push({
                        ...
                        isDevelopment: trimmed.includes('group:') && trimmed.includes('development'),
                        filePath
                    });
                }
            }
     ...
     */
    private parseNuGetDependencies(content: string, filePath: string): Dependency[] {
        const dependencies: Dependency[] = [];
        const fileName = path.basename(filePath);
    ...
    private parsePodfile(content: string, filePath: string): Dependency[] {
        ...
        for (const line of lines) {
            ...
            if (trimmed.startsWith('pod ')) {
                const podMatch = trimmed.match(/pod\s+['"]([^'"]+)['"](?:\s*,\s*['"]([^'"]+)['"])?/);
                if (podMatch) {
                    dependencies.push({
                        ecosystem: 'cocoapods',
    ...
    getEcosystemForFile(filePath: string): string | null {
        ...
        for (const ecosystem of this.ecosystems) {
            if (ecosystem.files.some(pattern => {
                if (pattern.includes('*')) {
                    const regex = new RegExp(pattern.replace(/\*/g, '.*'));
                    return regex.test(fileName);
                }
