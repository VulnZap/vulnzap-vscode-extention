...
import { DependencyCache, DependencyScanResult, VulnerabilityInfo } from './dependencyCache';
...
export class DependencyScanner {
    ...
    private activeScanPromises = new Map<string, Promise<DependencyScanResult | null>>();
    ...
    async scanWorkspaceDependencies(): Promise<DependencyScanResult[]> {
        ...
        for (const folder of workspaceFolders) {
            try {
                ...
                vscode.window.showErrorMessage(`Failed to scan ${folder.name}: ${error}`);
            }
        }
     ...
     */
    async scanProjectDependencies(projectPath: string): Promise<DependencyScanResult | null> {
     ...
     */
    private async performProjectScan(projectPath: string, projectHash: string): Promise<DependencyScanResult | null> {
        try {
            // Extract dependencies from project files
            console.log(`Extracting dependencies from ${projectPath}`);
            const dependencies = await this.parser.scanFolderForDependencies(projectPath);
            ...
            console.log(`Found ${dependencies.length} dependencies in ${projectPath}`);

            // Check cache first
            const cachedResult = await this.cache.getCachedScanResult(projectHash, dependencies);
            if (cachedResult) {
            ...
            const scanResult = await this.performBatchScan(projectHash, dependencies, projectPath);
            ...
            vscode.window.showErrorMessage(`Dependency scan failed for ${projectPath}: ${error}`);
            return null;
        }
    }
    ...
    private async performBatchScan(
        ...
        const apiUrl = config.get<string>('vulnzapApiUrl', 'https://api.vulnzap.com');
        const timeout = config.get<number>('dependencyScanTimeout', 60000);

        if (!apiKey) {
            vscode.window.showErrorMessage('VulnZap API key not configured. Please configure your API key in settings.');
            return null;
        }
        ...
        try {
            ...
            console.log(`Sending batch scan request for ${dependencies.length} packages`);
            ...
            if (axios.isAxiosError(error)) {
                ...
                if (status === 401) {
                    vscode.window.showErrorMessage('Invalid VulnZap API key. Please check your configuration.');
                } else if (status === 429) {
                    vscode.window.showErrorMessage('API rate limit exceeded. Please try again later.');
                } else if (status && status >= 500) {
                    vscode.window.showErrorMessage('VulnZap API server error. Please try again later.');
                } else {
                    vscode.window.showErrorMessage(`API request failed: ${error.response?.statusText || error.message}`);
                }
            } else {
                vscode.window.showErrorMessage(`Network error during vulnerability scan: ${error}`);
            }
    ...
    private processBatchScanResponse(
        ...
        console.log(`Scan completed for ${projectPath}: ${vulnerabilities.length} vulnerabilities found across ${vulnerablePackages} packages`);
        return scanResult;
    }
     ...
     */
    private extractVulnerabilitiesFromDataSources(item: ScanResultItem): VulnerabilityInfo[] {
        const vulnerabilities: VulnerabilityInfo[] = [];
        ...
        for (const vuln of allSources) {
            if (!uniqueVulns.has(vuln.cveId) || uniqueVulns.get(vuln.cveId)!.references.length < vuln.references.length) {
     ...
     */
    private generateRecommendation(vuln: VulnerabilityData, pkg: PackageInfo): string {
    ...
    private showScanResults(scanResult: DependencyScanResult): void {
        ...
        if (this.dependencyDiagnosticProvider) {
            ...
            if (workspaceFolders && workspaceFolders.length > 0) {
                ...
                this.dependencyDiagnosticProvider.updateDependencyDiagnostics(scanResult, projectPath);
            }
        }
     ...
     */
    private async showDetailedResults(scanResult: DependencyScanResult): Promise<void> {
    ...
    async onFileSaved(document: vscode.TextDocument): Promise<void> {
        ...
        if (timeSinceLastScan < debounceTime) {
            console.log(`Skipping scan for ${projectPath} - too soon since last scan (${timeSinceLastScan}ms < ${debounceTime}ms)`);
            return;
        }
     ...
     */
    getScanStatus(projectPath: string): 'scanning' | 'completed' | 'never-scanned' {
        const projectHash = DependencyCache.generateProjectHash(projectPath);
