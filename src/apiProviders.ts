          ...
          messages: [
            {
              role: "system",
              content:
                "You are a fast security scanner. Focus on critical vulnerabilities only. Respond with concise JSON.",
            },
            {
              role: "user",
              content: prompt,
          ...
          messages: [
            {
              role: "system",
              content:
                "You are a security expert analyzing code for vulnerabilities. Respond only with valid JSON in the specified format.",
            },
            {
              role: "user",
              content: prompt,
...
export class VulnZapProvider implements APIProvider {
  ...
  isConfigured(): boolean {
    // If session exists in global state, consider configured
    if (this.context) {
  ...
  async analyzeCode(
    ...
    const config = vscode.workspace.getConfiguration("vulnzap");
    let apiKey = config.get<string>("vulnzapApiKey");
    let accessToken = "";
    let baseUrl = config.get<string>(
      "vulnzapApiUrl",
    ...
    // Prefer access_token from session, fallback to API key
    ...
    if (!authHeader || !baseUrl) {
      throw new Error("VulnZap API key or session not configured");
    }
    ...
    try {
      ...
      if (error.response?.status === 401) {
        throw new Error("Invalid VulnZap API key or session");
  ...
  private validateAndNormalizeResponse(data: any): AISecurityResponse {
    ...
    if (!responseData.issues || !Array.isArray(responseData.issues)) {
      throw new Error(
        "Invalid VulnZap API response format: missing issues array"
      );
    }
...
export class APIProviderManager {
  ...
  private initializeProviders() {
    const providers = [
      ...
      new VulnZapProvider(this.context),
    ];
  ...
  setContext(context: vscode.ExtensionContext) {
    this.context = context;
    // Update VulnZapProvider with new context
    const vulnzapProvider = this.providers.get("vulnzap");
    if (vulnzapProvider && "setContext" in vulnzapProvider) {
      (vulnzapProvider as any).setContext(context);
    }
  }
  ...
  async analyzeCode(
    ...
    if (!this.currentProvider) {
      throw new Error(
        "No API provider selected. Please configure an API provider in settings."
      );
    }
    ...
    if (!this.currentProvider.isConfigured()) {
      ...
      throw new Error(
        `${
          this.currentProvider.displayName
        } is not properly configured: ${errors.join(", ")}`
      );
    }
