...
export function activate(context: vscode.ExtensionContext) {
    try {
        ...
        const documentChangeListener = vscode.workspace.onDidChangeTextDocument(async (event) => {
            ...
            const delay = vscode.workspace.getConfiguration('vulnzap').get('scanDelay', 1000);
            const enableFastScan = vscode.workspace.getConfiguration('vulnzap').get('enableFastScan', true);
            
            // Show immediate basic feedback if fast scan is enabled
            if (enableFastScan) {
                // Quick pattern-based scan (immediate feedback)
                setTimeout(async () => {
                    const basicIssues = await securityAnalyzer.fallbackToBasicAnalysis(document);
        ...
        const enableCommand = vscode.commands.registerCommand('vulnzap.enable', () => {
            console.log('VulnZap: Enable command called');
            isEnabled = true;
            vscode.workspace.getConfiguration('vulnzap').update('enabled', true, true);
            updateStatusBar();
        ...
        const disableCommand = vscode.commands.registerCommand('vulnzap.disable', () => {
            console.log('VulnZap: Disable command called');
            isEnabled = false;
            vscode.workspace.getConfiguration('vulnzap').update('enabled', false, true);
            updateStatusBar();
        ...
        const toggleCommand = vscode.commands.registerCommand('vulnzap.toggle', () => {
        ...
        const scanFileCommand = vscode.commands.registerCommand('vulnzap.scanFile', async () => {
        ...
        const selectApiProviderCommand = vscode.commands.registerCommand('vulnzap.selectApiProvider', async () => {
            ...
            try {
                ...
                vscode.window.showErrorMessage('Error selecting API provider: ' + (error as Error).message);
            }
        });

        const configureApiKeysCommand = vscode.commands.registerCommand('vulnzap.configureApiKeys', async () => {
            ...
            try {
                ...
                vscode.window.showErrorMessage('Error configuring API keys: ' + (error as Error).message);
            }
        });

        // Security View Commands
        const refreshSecurityViewCommand = vscode.commands.registerCommand('vulnzap.refreshSecurityView', () => {
        ...
        const clearAllIssuesCommand = vscode.commands.registerCommand('vulnzap.clearAllIssues', () => {
        ...
        const scanWorkspaceCommand = vscode.commands.registerCommand('vulnzap.scanWorkspace', async () => {
            console.log('VulnZap: Scan workspace command called');
            
            if (!isEnabled) {
                vscode.window.showWarningMessage('Security scanning is disabled. Enable it first.');
                return;
            }
            ...
            await vscode.window.withProgress({
                ...
                vscode.window.showInformationMessage(`Workspace scan complete. Scanned ${scannedCount} files.`);
            });
        });
        ...
        function updateStatusBar(status: string = '') {
            if (isEnabled) {
                if (status === 'quick-scan') {
                    ...
                    statusBarItem.tooltip = "Quick security scan completed. Full AI analysis in progress...";
        ...
        vscode.window.showErrorMessage('Error activating the extension: ' + (error as Error).message);
    }
}
